...
from scripts.eval.eval_rag_metrics import (
    ablation_metrics as _metric_ablation_metrics,
    aggregate_citation_scores as _metric_aggregate_citation_scores,
    build_ablation_summary as _metric_build_ablation_summary,
    build_retrieval_compare_summary as _metric_build_retrieval_compare_summary,
    evaluate_citation_quality as _metric_evaluate_citation_quality,
    finalize_citation_metrics as _metric_finalize_citation_metrics,
)
from scripts.eval.eval_rag_artifacts import (
    sanitize_citations as _sanitize_citations,
    sanitize_kg_expansions as _sanitize_kg_expansions,
    sanitize_kg_paths as _sanitize_kg_paths,
    sanitize_retrieved_docs as _sanitize_retrieved_docs,
)
from scripts.eval.eval_rag_reporting import build_eval_markdown_lines
...
def _evaluate_citation_quality(
    ...
    return _metric_evaluate_citation_quality(result, reference_sections)


def _finalize_citation_metrics(counts: Mapping[str, int], num_items: int) -> dict[str, object]:
    return _metric_finalize_citation_metrics(counts, num_items)


def _aggregate_citation_scores(scores: Sequence[CitationScore]) -> dict[str, object]:
    return _metric_aggregate_citation_scores(scores)
...
def evaluate_dataset(
    ...
    *,
    ...
    lines = build_eval_markdown_lines(
        accuracy=accuracy,
        label_accuracy=label_accuracy,
        unanswerable_accuracy=unanswerable_accuracy,
        grounded_rate=grounded_rate,
        avg_latency=avg_latency,
        dataset_id=dataset_id,
        dataset_task=dataset_meta.get("task"),
        provider=provider,
        model=model,
        num_items=num_items,
        top_k=top_k,
        ...
        kg_digest=kg_digest,
        evidence_coverage_recall=evidence_coverage_recall,
        multihop_items=multihop_items,
        multihop_evidence_coverage_recall=multihop_evidence_coverage_recall,
        kg_path_usage_rate=kg_path_usage_rate,
        multihop_trace_pass_rate=multihop_trace_pass_rate,
        citation_presence_rate=float(citation_metrics["presence_rate"]),
        citation_valid_citation_rate=float(citation_metrics["valid_citation_rate"]),
        citation_supported_rate=float(citation_metrics["supported_rate"]),
        citation_overclaim_rate=float(citation_metrics["overclaim_rate"]),
        citation_micro_precision=float(citation_pr["micro"]["precision"]),
        citation_micro_recall=float(citation_pr["micro"]["recall"]),
        citation_micro_f1=float(citation_pr["micro"]["f1"]),
        fallbacks_used=fallbacks_used,
        fallback_max_uses=fallback_max_uses,
        fallback_threshold_breached=fallback_threshold_breached,
        fallback_counts_dict=fallback_counts_dict,
        by_task=by_task,
...
def _ablation_metrics(payload: Mapping[str, object]) -> dict[str, float]:
    return _metric_ablation_metrics(payload)
...
def _build_ablation_summary(
    *,
    ...
    return _metric_build_ablation_summary(
        dataset_id=dataset_id,
...
def _build_retrieval_compare_summary(
    *,
    ...
    return _metric_build_retrieval_compare_summary(
        dataset_id=dataset_id,
        slice_definition=slice_definition,
        run_id=run_id,
        manifest_path=manifest_path,
