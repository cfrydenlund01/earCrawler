...
attached_src = root / 'Explainable Regulatory LLMs_ Current Landscape and Strategic Roadmap.docx'
if not attached_src.exists():
    attached_src = research_dir / 'Explainable Regulatory LLMs_ Current Landscape and Strategic Roadmap.docx'
...
if redlined_src.exists():
    try:
        ...
        for p in doc.paragraphs:
            t = (p.text or '').strip()
            if 'Addendum - Windows-First Interim Phase' in t or t.startswith('Dependencies: Phases C-F'):
...
outline_core = {
    ...
    "research_axes": {
        "models": [
            {"type": "Llama 3.1 8B/70B", "finetune": ["LoRA", "QLoRA"], "explainability": ["saliency", "rationale extraction"]},
            {"type": "Mistral 7B", "finetune": ["QLoRA"], "explainability": ["attribution", "KG-path grounding"]},
            {"type": "Legal-specific BERT/LED", "use": ["reranking", "summarization"]}
        ],
        "kg_constructions": [
            {"scheme": "EAR ontology v1", "storage": "Jena TDB2", "reasoning": ["RDFS", "OWL Mini"]},
            {"scheme": "Entity-Paragraph graph", "links": ["cites", "wasDerivedFrom"], "profiles": ["export bundles"]}
        ],
        "retrieval": [
            {"index": "BM25+FAISS", "fusion": "reciprocal-rank fusion", "filters": ["entity type", "date"]},
            {"index": "Dense (E5/legal embeddings)", "hybrid": True, "cache": "feature+snippet"}
        ]
    },
    "workstreams": [
        {"name": "Corpus & Provenance", "tasks": ["curate JSONL", "dedupe/merge", "redaction", "snapshots"], "prompt": "Implement corpus build/validate/snapshot CLI with deterministic outputs and provenance checks."},
        {"name": "KG & SPARQL", "tasks": ["freeze shapes", "export verification", "latency budgets"], "prompt": "Enforce export profiles and SPARQL perf warmers with CI gates."},
        {"name": "RAG & Agent", "tasks": ["index builder", "ranking policy", "SPARQL tool-use", "tracing"], "prompt": "Expose a RAG endpoint aligned with OpenAPI; include sources, scores, lineage."},
        {"name": "Eval & Explainability", "tasks": ["task suite", "benchmarks", "hallucination checks", "manuscript artifacts"], "prompt": "Create evaluation gates and exportable figures/tables for publication."}
    ],
    "manuscript_plan": {"tracks": [{"title": "Explainable RAG for EAR", "venue": "KDD/WWW/ACL Industry", "artifacts": ["datasets", "code", "appendix"]}, {"title": "KG-anchored QA for Regulations", "venue": "ISWC/Semantic Web Journal"}]},
    "acceptance": ["All tests pass", "Deterministic corpora", "Perf budgets met", "Reproducible eval"],
    "milestones": ["M1 Windows-first demo", "M2 KG+API", "M3 Finetune baselines", "M4 RAG+Eval", "M5 Release"]
}

variant_outlines = [
    {"title": "Hybrid RAG with KG Grounding", "hypotheses": ["Hybrid retrieval with entity filters improves precision", "Attribution to KG paths reduces hallucination"], "methods": {"retrieval": "BM25 + dense (E5-legal) with RRF", "grounding": "SPARQL templates for lineage and citations", "generation": "Mistral 7B QLoRA with chain-of-thought (regulated)"}, "datasets": ["EAR corpus", "NSF cases", "synthetic Q/A pairs"], "evaluation": {"metrics": ["hit@k", "nDCG@k", "Exact match", "BLEU/ROUGE", "Attribution score"], "latency": {"p95_ms": 1500}}, "risk_mitigations": ["privacy redaction", "denylist filters", "rate limits"], "gpt5_prompts": ["Implement indexer pipeline and hybrid retrieval with deterministic tests.", "Add SPARQL grounding step emitting source+path metadata in responses."]},
    {"title": "Reasoning over KG with Lightweight Generators", "hypotheses": ["OWL Mini inference enables fewer tokens for QA"], "methods": {"reasoning": "Fuseki OWL Mini endpoint for ASK/CONSTRUCT", "generator": "Small T5/LED summarizer for finalization"}, "evaluation": {"metrics": ["accuracy", "token cost", "latency"]}, "gpt5_prompts": ["Wire inference service and test ASK queries", "Compose answers from CONSTRUCT graphs"]},
    {"title": "Instruction-Tuned Legal QA", "hypotheses": ["Domain instruction data reduces errors"], "methods": {"finetune": "QLoRA on Mistral/Llama", "safety": "policy-aware decoding"}, "datasets": ["curated Q/A from EAR/NSF"], "gpt5_prompts": ["Dataset builder with redaction/licensing checks", "Trainer config + smoke eval"]}
]
...
odoc.add_paragraph('These outlines are structured for peer-review manuscript preparation.')
plans = []
for v in variant_outlines:
    plan = {
        "title": v["title"],
        "concrete_steps": ["Define experimental design and fix seeds", "Build/validate indexes and corpora (deterministic)", "Implement endpoints/contracts and observability", "Run evaluation sweeps and export figures/tables", "Draft manuscript sections with methods/results"],
        "instructions_for_gpt5": ["Follow acceptance criteria and do not break existing tests", "Add targeted unit tests for new logic", "Prefer Windows-first paths and offline fixtures", "Emit patch with minimal diff and clear structure"],
        "acceptance": ["Existing + new tests pass", "Perf/latency budgets met", "Deterministic outputs", "Reproducible evaluation artifacts"]
    }
    plans.append(plan)

odoc.add_heading('Variant Working Plans (JSON)', level=2)
odoc.add_paragraph(json.dumps({"plans": plans}, indent=2))
scaffold = {"base_header": ["You are a senior coding agent working in this repo.", "Keep changes minimal and deterministic; Windows-first.", "Validate with tests; avoid network in tests."], "work_package_template": {"goal": "...", "scope": ["..."], "key_files": ["path:line"], "deliverables": ["code", "tests", "docs"], "acceptance": ["..."], "validation": ["pytest -q", "py -m earCrawler.cli ..."], "non_goals": ["..."]}}
