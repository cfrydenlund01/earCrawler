...
Write-Host "Running non-destructive restore drill for snapshot: $snapshot"
...
$restoreCandidates = @("config", "workspace", "audit", "spool")
if ($DryRun) {
...
$report = [ordered]@{
    schema_version = "windows-single-host-restore-drill.v1"
...
Write-Host "Restore drill report written: $reportPath"

if ($status -ne "pass") {
    throw "Restore drill failed. Missing files: $($missing.Count); mismatched files: $($mismatched.Count)."
}
