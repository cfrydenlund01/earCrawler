...
def write_prompt(outdir: Path, name: str, goal: str, scope: list[str], key_files: list[str], deliverables: list[str], acceptance: list[str], validation_cmds: list[str], non_goals: list[str]):
...
def main():
    ap = argparse.ArgumentParser(description='Generate GPT-5 Codex prompts from working_json_outline')
    ap.add_argument('--outline', default='Research/working_json_outline.json')
    ap.add_argument('--outdir', default='Research/prompts')
    ap.add_argument('--all', action='store_true')
    ap.add_argument('--target', choices=['kg_shacl','kg_temporal','reasoning','retrieval','evaluation','policy_graph'])
    args = ap.parse_args()
    ...
    def gen_policy_graph():
        ...
        write_prompt(outdir, 'policy_graph', goal, scope, KEY_FILES['kg'], ['Updated ontology+shapes', 'Tests for SHACL/temporal'], acceptance, validation, ['Model training', 'API surface changes'])

    def gen_kg_shacl():
        shacl = w.get('kg_construction',{}).get('policy_graph',{}).get('constraints',{}).get('shacl',[])
        goal = 'Add SHACL shapes capturing policy-graph constraints and wire to integrity checks.'
        scope = [f'SHACL: {s}' for s in shacl] or ['Author SHACL shapes per policy-graph constraints']
        acceptance = ['All shapes conform on positive fixtures', 'Violations reported with clear messages']
        validation = ['pytest -q tests/kg/test_shacl_entities.py tests/test_shacl_parts.py']
        write_prompt(outdir, 'kg_shacl', goal, scope, KEY_FILES['kg'], ['shapes.ttl updates', 'tests'], acceptance, validation, ['Temporal logic'])

    def gen_kg_temporal():
        goal = 'Implement temporal validation and query patterns for EffectiveDate windows.'
        scope = ['Add temporal validation helper', 'Update integrity checks', 'Provide SPARQL examples']
        acceptance = ['Temporal constraints enforced on queries and TTL', 'Tests cover edge cases']
        validation = ['pytest -q tests/kg']
        write_prompt(outdir, 'kg_temporal', goal, scope, KEY_FILES['kg'], ['temporal utils', 'tests'], acceptance, validation, ['SHACL authoring'])

    def gen_reasoning():
        templates = w.get('kg_construction',{}).get('policy_graph',{}).get('reasoning_templates',[])
        goal = 'Implement reasoning templates over KG (obligations, permissions, procedures)'
        scope = [f'Implement template: {t}' for t in templates] or ['Implement obligation and exception chain templates']
        acceptance = ['Templates return correct paths on fixtures', 'Explainability payload includes KG paths']
        validation = ['pytest -q tests/kg tests/test_lineage.py']
        key_files = KEY_FILES['kg'] + KEY_FILES['sparql_api']
        write_prompt(outdir, 'reasoning_templates', goal, scope, key_files, ['SPARQL templates', 'tests'], acceptance, validation, ['Model training'])

    def gen_retrieval():
        r = w.get('retrieval_and_reasoning',{}).get('hybrid_retrieval',[])
        goal = 'Add hybrid retrieval with entity/date filters and optional dense fusion.'
        scope = r or ['BM25 retriever', 'Dense encoder integration', 'RRF fusion', 'Caching and determinism']
        acceptance = ['Deterministic retrieval on fixtures', 'Latency budgets documented']
        validation = ['pytest -q tests/rag/test_retriever.py']
        write_prompt(outdir, 'retrieval', goal, scope, KEY_FILES['retrieval'], ['retriever implementation', 'tests'], acceptance, validation, ['Model finetuning'])
    ...
    def gen_evaluation():
        ...
        acceptance = w.get('acceptance_criteria', ['Thresholds documented and enforced in tests'])
        validation = ['pytest -q tests/perf tests/eval -q']
        key_files = KEY_FILES['retrieval'] + KEY_FILES['kg']
        write_prompt(outdir, 'evaluation', goal, scope, key_files, ['eval harness', 'fixtures', 'reports'], acceptance, validation, ['Live API eval'])

    if args.all or args.target is None:
        gen_policy_graph(); gen_kg_shacl(); gen_kg_temporal(); gen_reasoning(); gen_retrieval(); gen_evaluation()
    else:
        {'kg_shacl': gen_kg_shacl, 'kg_temporal': gen_kg_temporal, 'reasoning': gen_reasoning, 'retrieval': gen_retrieval, 'evaluation': gen_evaluation, 'policy_graph': gen_policy_graph}[args.target]()
