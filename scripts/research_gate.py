...
def main():
    ap = argparse.ArgumentParser(description='Research gate: verify artifacts and run tests before advancing.')
    ap.add_argument('--name', required=True)
    ap.add_argument('--expect', action='append', default=[])
    ap.add_argument('--pytest', dest='pytest_args', nargs='*', default=[])
    ap.add_argument('--log', action='store_true', help='Append to Research/decision_log.md')
    ...
    if args.log:
        ...
        if dl.exists():
            ...
            getattr(mod, 'append_entry')(args.name, status, args.summary or f'pytest_rc={rc}', args.expect, env={})
